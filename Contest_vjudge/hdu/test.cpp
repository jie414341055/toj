...
bool getStatus(string pid,string lang,string & result,string& ce_info,string &tu,string &mu) {
	...
	while (true) {
		...
		result=getResult(ts);
		/* minjie */
		db_client.update("toj.Contest_Status",
				BSON("run_ID" << temp.runid << "cid" << temp.cid),
	...
	if(result == "Accepted")
		db_client.update("toj.User",
				...
				BSON("$inc"<<BSON("solved_contest"<<1)),
				false,true);
			//upsert,multi
	return true;
}
...
void judge(string pid,string lang,string runid,string src) {
	...
	db_client.insert("toj.Contest_Code", BSON("cid" << temp.cid << "run_ID" << temp.runid << "code" << src));
